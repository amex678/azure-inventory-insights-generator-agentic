...
- `agent-output` ディレクトリは作成済みです。集計データを読んだら、組み込みの edit/write ツールを直接使い、`agent-output/azure-inventory-insights.html` を1回で作成してください。
- ファイルやディレクトリの作成にシェルを使わないでください。`mkdir`、`touch`、`tee`、ヒアドキュメント、`python`、`node`、`perl`、`ruby` などを試したり、利用可能なコマンドや書き込み先を探索したりしないでください。
...
- 200〜350文字程度の「総評」を置き、本文全体を `id="overall-assessment"` の `<div>` 要素で囲み、環境規模、最も大きいリスクの集中領域、優先対応理由を説明する
